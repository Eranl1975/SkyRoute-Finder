...
export const bookingService: IBookingService = {
  async bookFlight(_request: BookingRequest): Promise<BookingResult> {
  ...
  async bookHotel(_request: HotelBookingRequest): Promise<BookingResult> {
    assertBookingEnabled();
    // TODO: Implement hotel booking adapter
    return { success: false, errorMessage: 'Not implemented', attemptedAt: nowISO() };
  },

  async sendConfirmation(_ref: string, _email: string): Promise<void> {
    assertBookingEnabled();
